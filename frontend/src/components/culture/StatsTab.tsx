...
export default function StatsTab({ cultureId }: Props) {
  ...
  if (isLoading) return <div className="text-center py-12 text-gray-400">Chargement des stats…</div>
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
          ...
          <p className="text-xs text-gray-500 mt-1">Arrosages</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-yellow-600">{stats.intensites_lampe.length}</p>
          <p className="text-xs text-gray-500 mt-1">Réglages lampe</p>
        </div>
      </div>
